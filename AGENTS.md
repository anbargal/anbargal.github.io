...
- `index.html` — song catalog table. Search box, song detail overlay, URL deep-link filtering (`?tivs=…&name=…`), and the "create bhajan list" selection modal (auto-opens via `?create=true`).
- `bhajan-lists.html` — saved lists (render cards, visit counts, remove, clear all).
...
- `common.js` — `BhajanLists` namespace wrapping `localStorage["bhajan_lists"]`: `get()`, `save(lists)`, `clear()`, `recordVisit(url, name)`.
...
4. **Deep-link filter**: open `index.html?tivs=001,005&name=RefactorTest` → status `2 of 448 song(s)`, `#list-name-display` shows the name, `#overlay` stays hidden (no `create=true`).
5. **Clear filter**: with a deep-link filter active, click `#clear-filter` badge → status back to `448 song(s)`.

#### index.html — create bhajan list modal

6. **Auto-open**: open `index.html?create=true` → `#selection-overlay` loses `hidden`, `#selection-list` has 448 rows.
7. **Selection + count**: click first two `.song-check-row input` → `#selection-count` shows `2 song(s) selected`; `#select-all` → `448`, `#deselect-all` → `0`.
8. **Generate Link**: select 2 songs, set `#list-name-input` = "Test List", click `#selection-generate` → `#share-url` is `index.html?tivs=…&name=…` **with no `create=true`**; `localStorage.bhajan_lists` contains one entry `{name:"Test List", count:1}`.
...
13. **Create button**: click `+ Create Bhajan List` → URL becomes `index.html?create=true` and the selection modal auto-opens (case 6).
...
18. **recordVisit create-branch**: open a deep link whose URL is NOT yet stored (`?tivs=…&name=NewName`) → storage gains a `{name:"NewName", count:1}` entry.
...
- URL generation in `index.html` (`generateLink`) must never emit a `create=true` param.
