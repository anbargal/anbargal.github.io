...
- **`bhajan-lists.html`** — saved bhajan lists. Render list cards with visit counts, remove individual lists, or clear all. Create lists from the catalog's selection modal.
...
| Path | Purpose |
| --- | --- |
| `index.html` | Song catalog, search, song overlay, create-list selection modal |
| `bhajan-lists.html` | Saved bhajan lists page |
| `common.js` | Shared `BhajanLists` localStorage helpers (`get`/`save`/`clear`/`recordVisit`) |
...
- Keep generated share URLs free of `create=true`.
